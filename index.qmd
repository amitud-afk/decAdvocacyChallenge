---
title: "Optimizing Patrick's Auto Shop Productivity and Revenue"
...
```
-->
<!--## Visualizing Daily Productivity 📈

Let's create comprehensive visualizations to understand productivity patterns across all shops. Here are several views of the data showing all 250 observations:
...
# Create a boss status label for better readability
carsDF['Boss Status'] = carsDF['boss'].map({0: 'Boss Absent', 1: 'Boss Present'})
carsDF['Shop'] = 'Shop ' + carsDF['shopID'].astype(str)
```

### All 250 Observations: Productivity by Shop and Boss Presence

This visualization shows every single data point, making it easy to see patterns and outliers:

```{python}
#| label: fig-all-points
#| fig-cap: "All 250 daily productivity observations across shops, showing the impact of boss presence"
#| echo: false
#| fig-width: 14
#| fig-height: 8
...
# Create a stripplot with jitter to show all points
colors = {'Boss Absent': '#3498db', 'Boss Present': '#e74c3c'}
for status in ['Boss Absent', 'Boss Present']:
    subset = carsDF[carsDF['Boss Status'] == status]
    for shop_id in sorted(subset['shopID'].unique()):
        shop_data = subset[subset['shopID'] == shop_id]
        x_pos = shop_id - 0.15 if status == 'Boss Absent' else shop_id + 0.15
        y_values = shop_data['carsFixed'].values
        x_values = np.random.normal(x_pos, 0.03, size=len(y_values))  # Jitter for visibility
        ax.scatter(x_values, y_values, alpha=0.6, s=50, color=colors[status], 
                  label=status if shop_id == 1 else '', edgecolors='white', linewidth=0.5)

# Add mean lines for each shop and boss status
for shop_id in sorted(carsDF['shopID'].unique()):
    for status in ['Boss Absent', 'Boss Present']:
        subset = carsDF[(carsDF['shopID'] == shop_id) & (carsDF['Boss Status'] == status)]
        if len(subset) > 0:
            mean_val = subset['carsFixed'].mean()
            x_pos = shop_id - 0.15 if status == 'Boss Absent' else shop_id + 0.15
            ax.plot([x_pos - 0.1, x_pos + 0.1], [mean_val, mean_val], 
                   color=colors[status], linewidth=3, alpha=0.8, linestyle='--')

ax.set_xlabel('Shop ID', fontweight='bold', fontsize=12)
ax.set_ylabel('Cars Fixed per Day', fontweight='bold', fontsize=12)
ax.set_title('Daily Productivity Across All Shops: All 250 Observations\n(Boss Presence vs Absence)', 
             fontweight='bold', fontsize=14, pad=20)
ax.set_xticks(sorted(carsDF['shopID'].unique()))
ax.set_xticklabels([f'Shop {i}' for i in sorted(carsDF['shopID'].unique())])
ax.grid(True, alpha=0.3, linestyle='--')
ax.legend(title='Boss Status', title_fontsize=11, fontsize=10, framealpha=0.9)

# Add annotation for total observations
ax.text(0.02, 0.98, f'Total Observations: 250', transform=ax.transAxes,
...
### Productivity Distribution by Shop

This box plot shows the distribution and spread of productivity for each shop:

```{python}
#| label: fig-boxplot
#| fig-cap: "Productivity distribution across shops, comparing boss presence vs absence"
#| echo: false
#| fig-width: 14
#| fig-height: 6
...
# Create boxplot with all data points overlaid
boxplot_data = []
positions = []
labels = []
colors_list = []
...
for shop_id in shop_ids:
    for boss_status in [0, 1]:
        subset = carsDF[(carsDF['shopID'] == shop_id) & (carsDF['boss'] == boss_status)]
        if len(subset) > 0:
            boxplot_data.append(subset['carsFixed'].values)
            positions.append(shop_id - 0.2 + x_offset)
            labels.append(f'Shop {shop_id}\n{"Boss Present" if boss_status == 1 else "Absent"}')
            colors_list.append('#e74c3c' if boss_status == 1 else '#3498db')
            x_offset += 0.4
    x_offset = 0

bp = ax.boxplot(boxplot_data, positions=positions, widths=0.3, patch_artist=True,
...
ax.set_xlabel('Shop', fontweight='bold', fontsize=12)
ax.set_ylabel('Cars Fixed per Day', fontweight='bold', fontsize=12)
ax.set_title('Productivity Distribution by Shop: Impact of Boss Presence', 
             fontweight='bold', fontsize=14, pad=15)
ax.set_xticks(shop_ids)
ax.set_xticklabels([f'Shop {i}' for i in shop_ids])
ax.grid(True, alpha=0.3, axis='y', linestyle='--')

# Create custom legend
from matplotlib.patches import Patch
legend_elements = [Patch(facecolor='#3498db', alpha=0.6, label='Boss Absent'),
                  Patch(facecolor='#e74c3c', alpha=0.6, label='Boss Present')]
...
### Time Series View: Productivity Over Time

This shows how productivity varies day by day across all shops:

```{python}
#| label: fig-timeseries
#| fig-cap: "Daily productivity trends over time across all shops, showing all 250 observations"
#| echo: false
#| fig-width: 16
#| fig-height: 8

fig, axes = plt.subplots(5, 1, figsize=(16, 8), sharex=True, sharey=True)

shop_ids = sorted(carsDF['shopID'].unique())

for idx, shop_id in enumerate(shop_ids):
    shop_data = carsDF[carsDF['shopID'] == shop_id].copy()
    shop_data = shop_data.sort_values('observation')
    
    ax = axes[idx]
    
    # Plot boss absent days
    absent_data = shop_data[shop_data['boss'] == 0]
    ax.scatter(absent_data['observation'], absent_data['carsFixed'], 
              alpha=0.6, s=40, color='#3498db', label='Boss Absent', 
              edgecolors='white', linewidth=0.3)
    
    # Plot boss present days
    present_data = shop_data[shop_data['boss'] == 1]
    ax.scatter(present_data['observation'], present_data['carsFixed'], 
              alpha=0.7, s=50, color='#e74c3c', label='Boss Present', 
              marker='^', edgecolors='white', linewidth=0.3)
    
    # Add trend line
    z = np.polyfit(shop_data['observation'], shop_data['carsFixed'], 1)
    p = np.poly1d(z)
    ax.plot(shop_data['observation'], p(shop_data['observation']), 
           "k--", alpha=0.3, linewidth=1)
    
    ax.set_ylabel(f'Shop {shop_id}', fontweight='bold', fontsize=10, rotation=0, 
                 labelpad=30, va='center')
    ax.grid(True, alpha=0.3, linestyle='--')
    ax.set_ylim([0, max(carsDF['carsFixed']) * 1.1])
    
    if idx == 0:
        ax.legend(loc='upper right', framealpha=0.9, fontsize=9)
        ax.set_title('Daily Productivity Trends Over Time: All 250 Observations', 
                    fontweight='bold', fontsize=14, pad=15)
    if idx == len(shop_ids) - 1:
        ax.set_xlabel('Observation Number (Day)', fontweight='bold', fontsize=12)

plt.tight_layout()
plt.show()
```

### Summary Comparison: Average Productivity by Shop

A clear comparison of average productivity showing the boss impact:

```{python}
#| label: fig-comparison
#| fig-cap: "Average daily productivity by shop, comparing boss presence vs absence"
#| echo: false
#| fig-width: 12
#| fig-height: 6

# Calculate means
comparison_data = carsDF.groupby(['shopID', 'Boss Status'])['carsFixed'].mean().reset_index()
comparison_data = comparison_data.pivot(index='shopID', columns='Boss Status', values='carsFixed')
...
bars1 = ax.bar(x - width/2, comparison_data['Boss Absent'], width, 
              label='Boss Absent', color='#3498db', alpha=0.8, edgecolor='white', linewidth=1.5)
bars2 = ax.bar(x + width/2, comparison_data['Boss Present'], width, 
              label='Boss Present', color='#e74c3c', alpha=0.8, edgecolor='white', linewidth=1.5)
...
ax.set_title('Average Productivity by Shop: Boss Presence vs Absence', 
             fontweight='bold', fontsize=14, pad=15)
ax.set_xticks(x)
ax.set_xticklabels([f'Shop {i}' for i in comparison_data.index])
ax.legend(title='Boss Status', title_fontsize=11, fontsize=10, framealpha=0.9)
ax.grid(True, alpha=0.3, axis='y', linestyle='--')
...
ax1.set_xlabel('Priority Score (Impact × Opportunity)', fontsize=12, fontweight='bold')
...
for rank, (_, row) in enumerate(priority_df.iterrows(), 1):
    if row['impact'] > 0:
        ...
        print(f"   Current performance: {row['absent_mean']:.1f} cars/day → {row['present_mean']:.1f} cars/day with Patrick")
    else:
...
ax2_twin.bar(x_pos2, conf_df['margin_error'] * 2, width=0.3, alpha=0.3, 
            color='orange', label='Uncertainty Range (±)', edgecolor='black', linewidth=1)

ax2.set_xlabel('Shop ID', fontsize=12, fontweight='bold')
ax2.set_ylabel('Number of Days', fontsize=12, fontweight='bold')
ax2_twin.set_ylabel('Uncertainty Range (± cars/day)', fontsize=12, fontweight='bold', color='orange')
...
for _, row in conf_df.iterrows():
    ...
    print(f"Uncertainty: ±{row['margin_error']:.1f} cars/day")
    print(f"Data: {int(row['n_absent'])} days absent, {int(row['n_present'])} days present")
    if row['ci_lower'] > 0:
        print(f"nterpretation: We're confident this shop BENEFITS from Patrick")
    elif row['ci_upper'] < 0:
        print(f"nterpretation: We're confident this shop is HURT by Patrick")
    else:
...
## Simulation Analysis: Testing Different Scenarios 🔮
...
for scenario_name in scenarios:
    ...
    print(f"  - Uncertainty: ±${(p95_rev - p5_rev)/2:,.0f}")
...
print(f"\n• Best Shop Strategy: ${best_mean:,.0f} average (${best_mean - current_mean:+,.0f} vs current)")
print(f"• Worst Shop Strategy: ${worst_mean:,.0f} average (${worst_mean - current_mean:+,.0f} vs current)")
print(f"• Opportunity Cost: ${best_mean - worst_mean:,.0f} difference between best and worst")
print(f"• Best strategy beats current in {exceedance_prob['Best_Shop']:.1f}% of simulations")
print(f"• Worst strategy beats current in {exceedance_prob['Worst_Shop']:.1f}% of simulations")
print("="*70)
```
...
takeaways_text = f"""
::: {{.callout-tip}}
## 🔑 Key Takeaways from Simulation Analysis
...
- ✅ High probability of success ({exceedance_prob['Best_Shop']:.1f}% win rate)
- ✅ Significant revenue upside (${np.mean(year_end_revenue['Best_Shop']) - current_mean:+,.0f} annually)
- ✅ Manageable risk (narrow confidence intervals)
